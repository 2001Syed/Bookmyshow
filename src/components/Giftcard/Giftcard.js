...
const filteringGeneric = giftCards.filter((x)=>{
  return x.category == "generic";
})

function Giftcard() {
    const [activeBtn, setActiveBtn] = useState([false, false , false, false, false])
    function changeactiveBtn(index){
        const changeactive = activeBtn.map((ele,i)=>{
            if (index === i) {
                return !ele;
            }
            return false;
        });
        setActiveBtn(changeactive);
    }
  return (
    <div className='giftcard-container'>
      <div className='giftcard-img1'>
        <img src="https://assets-in.bmscdn.com/promotions/cms/creatives/1692874595339_fdf.jpg" alt="" />
      </div>
      <div className='egiftcard-div'>
        <p>E-GIFT CARDS</p>
      </div>
      <div className='giftcardpick-div'>
        <div className='pickgiftcard-div'>
          <p>Pick a card from one of our themes</p>
        </div>
        <section className='giftcard-allbtns'>
            <button className= {activeBtn[0] ? "giftcard-btn giftcard-active-btn" : 'giftcard-btn'} onClick={() => {
                changeactiveBtn(0);
            }}>All</button>
            <button className= {activeBtn[1] ? "giftcard-btn giftcard-active-btn" : 'giftcard-btn'} onClick={() => {
                changeactiveBtn(1);
            }}>Generic</button>
            <button className= {activeBtn[2] ? "giftcard-btn giftcard-active-btn" : 'giftcard-btn'} onClick={() => {
                changeactiveBtn(2);
            }}>Occasion</button>
            <button className= {activeBtn[3] ? "giftcard-btn giftcard-active-btn" : 'giftcard-btn'} onClick={() => {
                changeactiveBtn(3);
            }}>Festivals</button>
            <button className= {activeBtn[4] ? "giftcard-btn giftcard-active-btn" : 'giftcard-btn'} onClick={() => {
                changeactiveBtn(4);
            }}>Combos</button>
        </section>
        <div className='Giftcard-imgcards-div'>
          {
            giftCards.map((x)=>{
              return <img src={x.gift} alt="" className='Giftcard-imgcards'/>
            })
          }
        </div>
      </div>
